...
class ViewportEditorWorkspaceView(CanvasStructureEditorWorkspaceView):
    ...
    def _set_fixed_zoom(self, scale: float) -> None:
        self._zoom_mode = "fixed"
        self._fixed_zoom_scale = max(_MIN_ZOOM, min(_MAX_ZOOM, float(scale)))
        self._schedule_render()
    ...
    def _install_viewport_controls(self) -> None:
        ...
        ttk.Label(
            controls,
            textvariable=self.zoom_text_value,
            style="Status.TLabel",
            width=10,
            anchor="center",
    ...
    def _render(self) -> None:
        ...
        render_scale = min(desired_scale, 1.0)
        try:
            rendered = render_project_preview(
                project,
                self.session.assets,
                max_width=max(1, round(project.canvas.width * render_scale)),
                max_height=max(1, round(project.canvas.height * render_scale)),
            )
        ...
        if display_image.size != (display_width, display_height):
            resampling = (
                ...
                if desired_scale < rendered.scale
    ...
    def _draw_grid(self) -> None:
        ...
        if not self._grid_visible or project is None or self._preview_scale <= 0:
            return
        spacing = _GRID_BASE
        while spacing * self._preview_scale < 12:
            spacing *= 2
        while spacing * self._preview_scale > 80 and spacing > _GRID_BASE / 4:
            spacing /= 2
        major_every = 4
        columns = int(math.floor(project.canvas.width / spacing))
        rows = int(math.floor(project.canvas.height / spacing))
        top = self._preview_top
        bottom = top + project.canvas.height * self._preview_scale
        left = self._preview_left
        right = left + project.canvas.width * self._preview_scale
        for index in range(columns + 1):
            x = left + index * spacing * self._preview_scale
        ...
        for index in range(rows + 1):
            y = top + index * spacing * self._preview_scale
            self.canvas.create_line(
                left,
                y,
                right,
    ...
    def _draw_preview_line(self, x1: float, y1: float, x2: float, y2: float) -> None:
        super()._draw_preview_line(
            self.canvas.canvasx(x1),
            self.canvas.canvasy(y1),
            self.canvas.canvasx(x2),
            self.canvas.canvasy(y2),
        )
