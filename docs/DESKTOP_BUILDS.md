...
To build all targets and publish a permanent GitHub Release, provide a version such as `v0.1.1` in the `release_tag` input:
...
On Windows, optional AI packages are stored under:

```text
%LOCALAPPDATA%\BatikCraftStudio\ai-runtime\site-packages
```

Model files and optional AI packages remain separate from the program installer so application updates do not force users to download SDXL and AI frameworks again. No system Python or manual terminal dependency installation is required.
