...
Public installer filenames always include the application release version so downloads from different releases are easy to distinguish. For release `v0.3.2`:

| Platform | Workflow artifact | Installer file |
| --- | --- | --- |
| Windows x64 | `BatikCraftStudio-Installer-Windows-x64` | `BatikCraftStudio-v0.3.2-Setup-Windows-x64.exe` |
| Linux x64 | `BatikCraftStudio-Installer-Linux-x64` | `BatikCraftStudio-v0.3.2-Installer-Linux-x64.deb` |
| macOS Intel | `BatikCraftStudio-Installer-macOS-x64` | `BatikCraftStudio-v0.3.2-Installer-macOS-x64.dmg` |
| macOS Apple Silicon | `BatikCraftStudio-Installer-macOS-arm64` | `BatikCraftStudio-v0.3.2-Installer-macOS-arm64.dmg` |

The installed application keeps the stable internal identity **BatikCraft Studio** so a new release upgrades the same application instead of creating a conflicting second installation. Windows Installed Apps displays the explicit release name, such as **BatikCraft Studio v0.3.2**.
...
To build all targets and publish a permanent GitHub Release, provide a version such as `v0.3.2` in the `release_tag` input:
...
`BatikCraftStudio-v0.3.2-Setup-Windows-x64.exe` is an Inno Setup installer. It:
...
- keeps application-managed AI dependencies in a writable per-user data directory when the Program Files installation directory is not writable;
...
```bash
sudo apt install ./BatikCraftStudio-v0.3.2-Installer-Linux-x64.deb
```
...
On Windows, managed dependencies are stored under the BatikCraft Studio per-user application-data directory when the installation folder is not writable. Existing packages and runtime models beside the executable remain available as a read-only compatibility fallback.
